...
import { TokenContract, NocomStablePoolV1Contract } from '@nocom-v1/contracts/artifacts';
import { useBalance } from '@/hooks/useBalance';
import { useStableEscrow } from '@/hooks/useStableEscrow';
import { setStableEscrowData } from '@/lib/storage/stableEscrowStorage';
...
export default function StableCollateralizeModal({
  open,
  onClose,
  collateralTokenName,
  ...
  // Check for existing stable escrow contract
  const { escrowContract, isLoading: isEscrowLoading, refetch: refetchEscrow } = useStableEscrow(
  ...
  const ensureEscrowDeployed = async () => {
    ...
    if (escrowContract) {
      console.log('[StableCollateralizeModal] Using existing stable escrow:', escrowContract.address.toString());
      return escrowContract;
    }

    console.log('[StableCollateralizeModal] No stable escrow found - deploying new escrow');
    setProcessingStep('Creating private escrow...');

    // Deploy stable escrow contract via simulation queue to prevent IndexedDB transaction conflicts
    const { contract, secretKey } = await simulationQueue.enqueue(() =>
      deployStableEscrowContract(
        wallet,
        userAddress,
    ...
    if (!apiResponse.ok) {
      ...
      console.error('[StableCollateralizeModal] Liquidator API registration failed:', errorData);
    ...
    await refetchEscrow();
    console.log('[StableCollateralizeModal] Stable escrow cache updated');
  ...
  const handleCollateralize = async () => {
    ...
    try {
      ...
      console.log('Using stable escrow contract:', escrowContractInstance.address.toString());
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-sm text-text-muted mt-1">
                    Back your zUSD stablecoin position
                  </p>
                </div>
                <button
                  onClick={onClose}
              ...
              {!isEscrowLoading && !escrowContract && (
                <div className="p-4 bg-blue-900/20 border border-blue-900/50 rounded-lg">
                  <p className="text-sm text-blue-400">
                    <strong>First-time setup:</strong> A stable escrow contract will be deployed for this market on your first collateralization.
                  </p>
                </div>
              )}
