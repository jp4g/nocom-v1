...
type StableWithdrawCollateralModalProps = {
  open: boolean;
  onClose: () => void;
  collateralPosition: CollateralPosition | null;
  stableEscrowContract: NocomStableEscrowV1Contract | undefined;
...
export default function StableWithdrawCollateralModal({
  open,
  onClose,
  collateralPosition,
  stableEscrowContract,
  wallet,
  userAddress,
}: StableWithdrawCollateralModalProps) {
  ...
  // Get collateral price (e.g., ZEC price)
  ...
  const stablePrice = 10000n;

  // Get debt position for this stable market (same poolAddress)
  const debtPosition = useMemo(() => {
    if (!collateralPosition) return null;
    return portfolioData.debt.find(
      d => d.poolAddress === collateralPosition.poolAddress && d.isStable
    ) ?? null;
  ...
  // Use ZCASH_LTV for stable pools
  ...
  }, [currentCollateral, currentDebt, collateralPrice, maxLtv]);
  ...
  const currentLtvPercent = useMemo(() => {
    ...
    const debtValue = (currentDebt * stablePrice) / PRICE_BASE;
  ...
  }, [inputValue, currentCollateral, currentDebt, collateralPrice, maxLtv, currentHealthFactor]);
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      console.log("Withdrawing stable collateral:", {
        userAddress: userAddress.toString(),
        escrowContract: stableEscrowContract.address.toString(),
        amount,
        collateralPrice
      });
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-sm text-text-muted mt-1">
                    {collateralPosition.loanAsset.toUpperCase()} / {collateralPosition.collateralAsset.toUpperCase()} Stablecoin Market
                  </p>
                </div>
                <button
                  onClick={onClose}
            ...
            <div className="p-6">
              ...
              <div className="space-y-4">
                ...
                <button
                  ...
                  disabled={!isValidInput || !stableEscrowContract}
